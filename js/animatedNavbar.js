...
function moveIndicator(link) {
  ...
  const navbarRect = animatedNavMenu.getBoundingClientRect();
  ...
  const targetLeft = linkRect.left - navbarRect.left;
  ...
  const leftDifference = targetLeft - startLeft;
  const widthDifference = targetWidth - startWidth;
  ...
  function animate(currentTime) {
    ...
    currentLeft = startLeft + leftDifference * ease;

    // Calculate current width

    currentWidth = startWidth + widthDifference * ease;
...
function setCurrentPage() {
  ...
  if (activeLink && window.innerWidth > 1203) {
...
function setIndicatorPosition(link) {
  ...
  const navbarRect = animatedNavMenu.getBoundingClientRect();
  const linkRect = link.getBoundingClientRect();
  currentLeft = linkRect.left - navbarRect.left;
  currentWidth = linkRect.width;
  navIndicator.style.left = currentLeft + "px";
  navIndicator.style.width = currentWidth + "px";
}
...
animatedNavMenu.addEventListener("mouseleave", function () {
  if (window.innerWidth > 1203) {
    // Remove hovered class
...
navLinks.forEach((link) => {
  link.addEventListener("click", function () {
    ...
    if (window.innerWidth <= 1203) {
      animatedNavMenu.classList.remove("menu-open");
    }
  });
});
...
animatedMenuToggle.addEventListener("click", function () {
  animatedNavMenu.classList.toggle("menu-open");
});
